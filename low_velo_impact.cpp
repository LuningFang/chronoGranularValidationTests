...
void ShowUsage(std::string name) {
    std::cout << "usage: " + name + " <json_file> "  + " <input_file> " + " <drop height, 1 ~ 7 cm> " + " <gravity> "  <<std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    float initial_height = std::stof(argv[3]);
    float grav_z = std::stof(argv[4]);
    ...
    gran_sys.set_gravitational_acceleration(params.grav_X, params.grav_Y, grav_z);
    ...
    params.output_dir = params.output_dir+"_height_"+argv[3];
    ...
    for (double t = 0; t < (double)params.time_end; t += iteration_step, curr_step++) {
        ...
        if (ball_pos.z() - (initial_surface + ball_radius) < 1E-3){
            ...
            if (curr_step % cout_freq == 0){
                printf("t=%f, KE=%e J, penetration_d = %e cm, velo_ball = %e cm/s, acc_ball = %e cm/s^2\n", t, KE*1E-5, penetration_d, ball_body->GetPos_dt().z(), ball_body->GetPos_dtdt().z());
            }
        }
